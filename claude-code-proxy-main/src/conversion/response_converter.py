...
def handle_lmp_error_response(error_data: dict) -> dict:
    """Handle LMP error response format {code, success, message, data}."""
    error_code = error_data.get("code", "")
    success = error_data.get("success", False)
    message = error_data.get("message", "Unknown error")

    # Map LMP error codes to HTTP status codes
    if error_code in ["300001", "300002"]:
        status_code = 401
    elif any(error_code.startswith("20000") for error_code in ["200001", "200002", "200003", "200004", "200005"]):
        status_code = 400
    elif error_code in ["400001", "400002"]:
        status_code = 500
    else:
        status_code = 500
    ...
    return {
        ...
        "lmp_error_code": error_code,
        "lmp_success": success,
    }


def convert_openai_to_claude_response(
    openai_response: dict, original_request: ClaudeMessagesRequest, api_provider: str = "openai"
) -> dict:
    ...
    lmp_metadata = {}
    if api_provider == "lmp":
        app_id = openai_response.get("appId", "")
        global_trace_id = openai_response.get("globalTraceId", "")
        is_sensitive_word = openai_response.get("isSensitiveWord", False)
        lmp_metadata = {
            "appId": app_id,
            "globalTraceId": global_trace_id,
            "isSensitiveWord": is_sensitive_word,
        }
    ...
    # Add LMP metadata if present
    if lmp_metadata:
        claude_response["lmp_metadata"] = lmp_metadata
...
async def convert_openai_streaming_to_claude_with_cancellation(
    ...
    # Delegate to LMP V2 handler if in LMP V2 mode
    if api_provider == "lmp" and lmp_api_version == "V2":
        async for chunk in convert_lmp_v2_streaming_to_claude(
            openai_stream,
            original_request,
            logger,
            http_request,
...
async def convert_lmp_v2_streaming_to_claude(
    openai_stream,
    original_request: ClaudeMessagesRequest,
    logger,
    ...
    """Convert LMP V2 streaming response (no 'data:' prefix) to Claude streaming format."""
    ...
    try:
        async for line in openai_stream:
            # Check for client disconnection if http_request is provided
            if http_request:
                try:
                    if await http_request.is_disconnected():
                        logger.info(f"Client disconnected, cancelling request {request_id}")
            ...
            if line.strip():
                # LMP V2 format: no "data:" prefix, direct JSON
                ...
                try:
                    ...
                    # Extract LMP metadata from first chunk
                    if first_chunk:
                        app_id = chunk.get("appId", "")
                        global_trace_id = chunk.get("globalTraceId", "")
                        is_sensitive_word = chunk.get("isSensitiveWord", False)
                        lmp_metadata = {
                            "appId": app_id,
                            "globalTraceId": global_trace_id,
                            "isSensitiveWord": is_sensitive_word,
                    ...
                    logger.warning(f"Failed to parse LMP V2 chunk: {chunk_data}, error: {e}")
                    continue
    ...
    yield f"event: {Constants.EVENT_MESSAGE_DELTA}\ndata: {json.dumps({'type': Constants.EVENT_MESSAGE_DELTA, 'delta': {'stop_reason': final_stop_reason, 'stop_sequence': None}, 'usage': usage_data, 'lmp_metadata': lmp_metadata if lmp_metadata else {}}, ensure_ascii=False)}\n\n"
    yield f"event: {Constants.EVENT_MESSAGE_STOP}\ndata: {json.dumps({'type': Constants.EVENT_MESSAGE_STOP}, ensure_ascii=False)}\n\n"
